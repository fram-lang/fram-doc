...
The easiest way to start experimenting with Fram is to use the interactive mode (REPL).
If you have `dbl` installed (see [Installation](installation.md)), you can start it in the interactive mode by running the command without any arguments:
...
In the above example, the `+` operator resolves to a method `add` of the type of x. If the type of x is not specified, the interpreter cannot infer which method to use so we need to annotate it explicitly. 
...
Pattern matching is a powerful feature in Fram used to check a value against a pattern. It is most commonly used with [algebraic data types](data-types.md).
